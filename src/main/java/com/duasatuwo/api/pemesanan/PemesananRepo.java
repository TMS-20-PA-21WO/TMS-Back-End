...
public interface PemesananRepo extends CrudRepository<Pemesanan, Integer> {
    ...
    @Query(value = "SELECT a.* FROM pemesanan a WHERE a.id_user = :id_user AND a.date = :date", nativeQuery = true)
    public Iterable<Pemesanan> doValidation(@Param("id_user") int id_user, @Param("date") String date);
}